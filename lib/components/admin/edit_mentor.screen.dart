...
class _EditMentorScreenState extends State<EditMentorScreen> {
  ...
  void initState() {
    ...
    numberOfMentoreesController =
        TextEditingController(text: mentorData.numberOfMentoree.toString());
    rateController = TextEditingController(text: mentorData.rate.toString());
    freePriceController =
        TextEditingController(text: mentorData.free.price.toString());
    ...
    experienceCompanyControllers = mentorData.experiences
        .map(
            (experience) => TextEditingController(text: experience.companyName))
        .toList();
    ...
    experienceDescriptionControllers = mentorData.experiences
        .map(
            (experience) => TextEditingController(text: experience.description))
        .toList();
  }
  ...
  Future<void> saveMentor() async {
    ...
    for (int i = 0; i < mentorData.certificates.length; i++) {
      ...
      if (certificate.name.isEmpty ||
          certificate.provideBy.isEmpty ||
          certificate.createDate == null ||
          certificate.imageUrl.isEmpty) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content:
                Text('Please fill in all fields for certificate ${i + 1}.'),
            backgroundColor: Colors.red,
          ),
        );
    ...
    if (mentorData.timeSlots.isEmpty ||
        timeSlotsTimeStartControllers.isEmpty ||
        timeSlotsTimeEndControllers.isEmpty) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(
    ...
    for (int i = 0; i < timeSlotsTimeStartControllers.length; i++) {
      if (timeSlotsTimeStartControllers[i].text.isEmpty ||
          timeSlotsTimeEndControllers[i].text.isEmpty) {
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(
    ...
    if (isExpired) {
      ...
      if (response.statusCode == 200) {
        ...
        Navigator.pop(
            context, true); // This triggers the reload in the admin page
      } else {
        showDialog(
          context: context,
    ...
    setState(() {
      switch (field) {
        ...
        case 'bio':
          ...
          break;
        case 'rate':
          mentorData = mentorData.copyWith(
              rate: double.tryParse(value) ?? mentorData.rate);
          break;
        case 'numberOfMentoree':
          mentorData = mentorData.copyWith(
              numberOfMentoree:
                  value is num ? value.toInt() : mentorData.numberOfMentoree);
          break;
        case 'freePrice':
          mentorData = mentorData.copyWith(
              free: mentorData.free.copyWith(
                  price: (double.tryParse(value) ?? mentorData.free.price)
                      .toInt()));
          break;
        case 'freeUnit':
          mentorData =
              mentorData.copyWith(free: mentorData.free.copyWith(unit: value));
          break;
        case 'categories':
          if (index != null) {
            mentorData.categories[index] =
                mentorData.categories[index].copyWith(name: value);
          }
          break;
        case 'timeSlots':
          if (index != null) {
          ...
          if (index != null) {
            final updatedExperience = mentorData.experiences[index].copyWith(
              ...
              startDate: DateTime.tryParse(
                  ...
                  mentorData.experiences[index].startDate,
              endDate:
                  DateTime.tryParse(experienceEndDateControllers[index].text) ??
                      mentorData.experiences[index].endDate,
          ...
          onPressed: () async {
            final selectedCategories = await showDialog<List<Category>>(
              ...
              builder: (_) => MultiSelectDialog<Category>(
                items: allCategories
                    .map((category) =>
                        MultiSelectItem<Category>(category, category.name))
                    .toList(),
            ...
            ...mentorData.experiences.asMap().entries.map((entry) {
              ...
              return Padding(
                padding: const EdgeInsets.symmetric(
                    vertical: 8.0), // Optional: Add vertical spacing
                child: Container(
                  color: Colors.transparent, // Remove background color
                  padding:
                      const EdgeInsets.all(8.0), // Optional: Add padding inside
                  child: Column(
                    crossAxisAlignment: CrossAxisAlignment.start,
                    children: [
                      // Role
